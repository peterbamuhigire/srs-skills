...
**Dependencies:** `PDO`, `SessionService`, `AuditService`, `InventoryPlanningService`, `WorkCentreCalendarService`

| Method | Parameters | Returns | Description |
|---|---|---|---|
| `runMrp(int $planVersionId, array $scope): int` | Plan version, item/location/date scope | New `mrp_runs.id` | Explodes released BOMs, nets requirements against available supply, applies lot-sizing rules, and stores pegged planned orders. |
| `getPegging(int $plannedOrderId): array` | Planned order primary key | Pegging tree | Returns source demand, parent production order, BOM/routing versions, inventory position, and policy version behind the planned order. |
| `releasePlannedOrders(array $plannedOrderIds): array` | Planned order identifiers | Release result | Converts approved planned orders to purchase, production, or transfer proposals without duplicate downstream creation. |

**Tables read/written:** `mrp_runs`, `mrp_requirements`, `mrp_planned_orders`, `mrp_pegging_links`, `planning_exceptions`
...
**Dependencies:** `PDO`, `SessionService`, `AuditService`, `ManufacturingPlanningService`

| Method | Parameters | Returns | Description |
|---|---|---|---|
| `buildSchedule(array $scope): int` | Work-centre/date/order scope | New `production_schedule_runs.id` | Creates a feasible or exception-marked operation schedule using calendars, setup matrix, run time, material readiness, and priority rules. |
| `resequenceQueue(int $workCentreId, array $operationIds): array` | Work centre, ordered operation identifiers | Updated queue summary | Recalculates setup time, projected completion, and lateness after manual resequencing. |
| `captureResourceIntensity(int $operationId, array $readings): void` | Operation identifier, energy/water/runtime readings | `void` | Stores energy, water, machine-hour, labour-hour, scrap, rework, and recovery readings for KPI and variance calculations. |

**Tables read/written:** `production_schedule_runs`, `production_operation_schedule`, `work_centre_calendars`, `setup_matrices`, `production_resource_readings`, `production_green_kpis`
...
**Module guard:** `ModuleRegistry::isActive('INVENTORY', $tenantId)`

**Dependencies:** `PDO`, `SessionService`, `AuditService`, `InventoryService`

| Method | Parameters | Returns | Description |
|---|---|---|---|
| `recommendPutaway(array $receiptLine): array` | Receipt, transfer, or production receipt line | Candidate bin list | Scores bins using storage policy, available capacity, rotation rule, quarantine status, and congestion. |
| `createForwardPickReplenishment(int $itemId, int $warehouseId): int` | Item and warehouse identifiers | New replenishment task ID | Creates reserve-to-forward-pick replenishment when trigger quantity is breached. |
| `generatePickPath(int $waveId): array` | Pick wave identifier | Ordered pick tasks | Sequences picks by zone, aisle, bin, FEFO/FIFO rule, and order priority. |

**Tables read/written:** `warehouse_storage_policies`, `warehouse_bins`, `warehouse_tasks`, `warehouse_pick_waves`, `warehouse_task_events`
