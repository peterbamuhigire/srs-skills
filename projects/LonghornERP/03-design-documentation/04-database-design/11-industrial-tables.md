...
# MRP, Scheduling, and Green Production Tables

## `mrp_runs`

Planning-run header for material requirements planning.

| Column | Type | Constraints | Description |
|---|---|---|---|
| `id` | BIGINT UNSIGNED | PRIMARY KEY, AUTO_INCREMENT | Surrogate primary key. |
| `tenant_id` | BIGINT UNSIGNED | NOT NULL, FK -> `tenants.id` | Tenant scope. |
| `plan_version_id` | BIGINT UNSIGNED | NOT NULL | Demand or supply plan version used. |
| `horizon_start` | DATE | NOT NULL | First planning date. |
| `horizon_end` | DATE | NOT NULL | Last planning date. |
| `status` | ENUM('running','completed','failed','cancelled') | NOT NULL, DEFAULT 'running' | Run state. |
| `started_at` | DATETIME | NOT NULL, DEFAULT CURRENT_TIMESTAMP | Start time. |
| `completed_at` | DATETIME | NULL | Completion time. |

**Indexes:** PRIMARY (`id`), (`tenant_id`, `status`, `started_at`).

---

## `mrp_planned_orders`

Planned buy, make, or transfer recommendations created by MRP.

| Column | Type | Constraints | Description |
|---|---|---|---|
| `id` | BIGINT UNSIGNED | PRIMARY KEY, AUTO_INCREMENT | Surrogate primary key. |
| `tenant_id` | BIGINT UNSIGNED | NOT NULL, FK -> `tenants.id` | Tenant scope. |
| `mrp_run_id` | BIGINT UNSIGNED | NOT NULL, FK -> `mrp_runs.id` | Source run. |
| `item_id` | BIGINT UNSIGNED | NOT NULL, FK -> `stock_items.id` | Planned item. |
| `branch_id` | BIGINT UNSIGNED | NOT NULL, FK -> `branches.id` | Planned location. |
| `order_type` | ENUM('buy','make','transfer') | NOT NULL | Planned order type. |
| `need_date` | DATE | NOT NULL | Date the supply is needed. |
| `release_date` | DATE | NOT NULL | Suggested release date after lead-time offset. |
| `quantity` | DECIMAL(15,4) | NOT NULL | Recommended quantity after lot sizing. |
| `lot_sizing_rule` | VARCHAR(40) | NOT NULL | Rule applied to net requirement. |
| `status` | ENUM('planned','firm','released','cancelled') | NOT NULL, DEFAULT 'planned' | Planner state. |

**Indexes:** PRIMARY (`id`), (`tenant_id`, `mrp_run_id`), (`tenant_id`, `item_id`, `branch_id`, `need_date`), (`tenant_id`, `status`, `release_date`).

---

## `mrp_pegging_links`

Links planned orders to source demand and parent requirements.

| Column | Type | Constraints | Description |
|---|---|---|---|
| `id` | BIGINT UNSIGNED | PRIMARY KEY, AUTO_INCREMENT | Surrogate primary key. |
| `tenant_id` | BIGINT UNSIGNED | NOT NULL, FK -> `tenants.id` | Tenant scope. |
| `planned_order_id` | BIGINT UNSIGNED | NOT NULL, FK -> `mrp_planned_orders.id` | Planned order. |
| `source_type` | VARCHAR(50) | NOT NULL | Demand, production order, forecast, transfer, or safety-stock source. |
| `source_id` | BIGINT UNSIGNED | NOT NULL | Source record identifier. |
| `source_quantity` | DECIMAL(15,4) | NOT NULL | Quantity pegged from source. |

**Indexes:** PRIMARY (`id`), (`tenant_id`, `planned_order_id`), (`tenant_id`, `source_type`, `source_id`).

---

## `work_centre_calendars`

Capacity calendar for production work centres.

| Column | Type | Constraints | Description |
|---|---|---|---|
| `id` | BIGINT UNSIGNED | PRIMARY KEY, AUTO_INCREMENT | Surrogate primary key. |
| `tenant_id` | BIGINT UNSIGNED | NOT NULL, FK -> `tenants.id` | Tenant scope. |
| `work_centre_id` | BIGINT UNSIGNED | NOT NULL | Work centre identifier. |
| `calendar_date` | DATE | NOT NULL | Capacity date. |
| `shift_code` | VARCHAR(40) | NOT NULL | Shift identifier. |
| `available_minutes` | INT UNSIGNED | NOT NULL | Net available capacity. |
| `maintenance_block_minutes` | INT UNSIGNED | NOT NULL, DEFAULT 0 | Planned maintenance block. |
| `capacity_uom` | VARCHAR(20) | NULL | Optional output unit of measure. |

**Indexes:** PRIMARY (`id`), UNIQUE (`tenant_id`, `work_centre_id`, `calendar_date`, `shift_code`).
...
| `tenant_id` | BIGINT UNSIGNED | NOT NULL, FK -> `tenants.id` | Tenant scope. |
| `work_centre_id` | BIGINT UNSIGNED | NOT NULL | Work centre identifier. |
| `from_family_code` | VARCHAR(60) | NOT NULL | Previous product family. |
| `to_family_code` | VARCHAR(60) | NOT NULL | Next product family. |
| `setup_minutes` | INT UNSIGNED | NOT NULL | Expected setup/changeover minutes. |
| `setup_cost` | DECIMAL(18,2) | NOT NULL, DEFAULT 0 | Optional setup cost. |

**Indexes:** PRIMARY (`id`), UNIQUE (`tenant_id`, `work_centre_id`, `from_family_code`, `to_family_code`).

---

## `production_resource_readings`

Operation-level resource and sustainability readings.

| Column | Type | Constraints | Description |
|---|---|---|---|
| `id` | BIGINT UNSIGNED | PRIMARY KEY, AUTO_INCREMENT | Surrogate primary key. |
| `tenant_id` | BIGINT UNSIGNED | NOT NULL, FK -> `tenants.id` | Tenant scope. |
| `production_order_id` | BIGINT UNSIGNED | NOT NULL | Production order. |
| `operation_id` | BIGINT UNSIGNED | NULL | Operation or job-card line. |
| `energy_kwh` | DECIMAL(15,4) | NULL | Energy used. |
| `water_litres` | DECIMAL(15,4) | NULL | Water used. |
| `machine_minutes` | INT UNSIGNED | NULL | Machine runtime. |
| `labour_minutes` | INT UNSIGNED | NULL | Labour time. |
| `scrap_qty` | DECIMAL(15,4) | NULL | Scrap quantity. |
| `rework_qty` | DECIMAL(15,4) | NULL | Rework quantity. |
| `recorded_at` | DATETIME | NOT NULL, DEFAULT CURRENT_TIMESTAMP | Reading time. |

**Indexes:** PRIMARY (`id`), (`tenant_id`, `production_order_id`), (`tenant_id`, `operation_id`, `recorded_at`).
