...
## 9.6 Finite Scheduling, Changeover, and Green Production

**FR-MFG-055** - The system shall maintain work-centre calendars that define available shifts, planned breaks, planned maintenance, rated capacity, setup crew requirement, and eligible operation types. Production dispatch shall reject assignment to a work centre whose calendar is unavailable unless an authorised supervisor records an override reason.

**FR-MFG-056** - The system shall support sequence-dependent setup and changeover rules by work centre and product family. When a supervisor resequences an operation queue, the system shall recalculate expected setup time, projected completion time, and lateness risk.

**FR-MFG-057** - The production schedule board shall rank dispatch candidates using configurable priority factors, including due date, customer priority, material availability, setup minimisation, work-centre load, quality hold status, and expected lateness penalty.

**FR-MFG-058** - The system shall capture production resource intensity per order and operation, including energy use, water use, machine hours, labour hours, material yield, scrap quantity, rework quantity, and by-product recovery where meters or manual readings are available.

**FR-MFG-059** - The system shall calculate green production KPIs per product, batch, work centre, and period, including energy per unit, water per unit, yield percentage, scrap percentage, rework percentage, recovered by-product value, and avoidable waste cost.

**FR-MFG-060** - When a production order is closed, the system shall compare actual setup time, run time, yield, scrap, energy use, and water use against the approved routing or recipe standard and shall generate variance records for values outside tenant-defined tolerance.

**FR-MFG-061** - The system shall provide a simulation-ready export for production scenarios containing routing, work-centre calendars, operation times, setup matrix, material constraints, planned demand, and current WIP so advanced users can run external optimisation without manually rebuilding the factory model.
