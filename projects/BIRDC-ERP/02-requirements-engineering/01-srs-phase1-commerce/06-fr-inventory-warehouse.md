...
**FR-INV-038** — Warehouse Slot and Capacity Management

When an IT Administrator or Store Manager defines storage locations, the system shall support zone, aisle, rack, bin, storage medium, maximum weight, maximum volume, maximum pallet count, cold-store flag, and active/inactive status. Putaway, picking, and stock-count workflows shall reference the bin where bin tracking is active.

*Traceability: BG-001, BG-002, DC-002, DC-003.*

---

**FR-INV-039** — Policy-Based Putaway Recommendation

When goods are received from procurement or production, the system shall recommend a putaway bin using product storage policy, available bin capacity, expiry or FIFO rotation, QC status, cold-store requirement, and distribution staging priority. The Store Manager may override the recommendation only with a reason code.

*Traceability: BG-001, BG-002, DC-003.*

---

**FR-INV-040** — Forward-Pick Replenishment

When a forward-pick location for a finished good or packaging item falls below the configured trigger quantity, the system shall create a replenishment task from reserve storage to the forward-pick bin, showing source bin, destination bin, required quantity, due time, and priority.

*Traceability: BG-001, DC-001, DC-003.*

---

**FR-INV-041** — Pick Path and Staging Control

When warehouse staff pick stock for sales, agent issuance, production material issue, or transfer, the system shall generate a pick sequence using zone, aisle, bin, FEFO rule, batch status, and order priority; picked stock shall move to a named staging location until dispatch or production issue is confirmed.

*Traceability: BG-001, BG-002, BG-003, BR-007, DC-003.*

---

**FR-INV-042** — Warehouse Productivity Dashboard

When the Store Manager views warehouse performance, the system shall show inventory accuracy, FEFO compliance, pick accuracy, dock-to-stock time, putaway cycle time, pick cycle time, storage utilisation, near-expiry staging stock, and unresolved warehouse task backlog by location and date range.

*Traceability: BG-001, BG-002, DC-001.*
