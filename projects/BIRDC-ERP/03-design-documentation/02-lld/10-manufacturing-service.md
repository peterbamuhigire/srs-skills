...
final class ManufacturingService
{
    ...
    /**
     * Check time-phased material availability before release or reschedule.
     *
     * @param int $orderId
     *
     * @return MaterialAvailabilityResult {
     *   order_id: int,
     *   required_date: DateTimeString,
     *   lines: [{stock_item_id, required_qty, on_hand_qty, reserved_qty, open_receipt_qty,
     *            quarantine_qty, net_available_qty, shortage_qty, suggested_action}],
     *   releasable: bool
     * }
     *
     * Business rules:
     *   - Uses active recipe version and planned order dates.
     *   - Excludes quarantine, rejected, and expired batches.
     *   - Creates manufacturing exceptions for shortages that block release.
     */
    public function checkMaterialAvailability(int $orderId): MaterialAvailabilityResult;

    /**
     * Build or refresh the operation schedule for a production order.
     *
     * @param int $orderId
     *
     * @return OperationScheduleResult {
     *   order_id: int,
     *   operations: [{operation_id, station_id, planned_start, planned_finish,
     *                 setup_minutes, run_minutes, capacity_status}],
     *   overloads: array
     * }
     *
     * Business rules:
     *   - Applies station calendar, rated capacity, setup/changeover rules, and planned maintenance.
     *   - Overloads require Production Manager override before release.
     */
    public function buildOperationSchedule(int $orderId): OperationScheduleResult;

    /**
     * Capture resource and sustainability readings at order or operation level.
     *
     * @param int   $orderId
     * @param array $readings  {operation_id, energy_kwh, water_litres, machine_minutes,
     *                          labour_minutes, scrap_qty, rework_qty, recovered_byproduct_qty}
     *
     * @return void
     *
     * Business rules:
     *   - Readings are append-only and feed production variance and sustainability reports.
     *   - Values below zero are rejected.
     */
    public function recordResourceReadings(int $orderId, array $readings): void;
}
```
