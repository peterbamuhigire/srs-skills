...
### tbl_workstation_calendars

**Purpose:** Capacity calendars for factory stations and equipment used by finite scheduling.

**Key columns:**

| Column | Type | Constraint | Description |
|---|---|---|---|
| `calendar_id` | `INT UNSIGNED` | PK, AUTO_INCREMENT | Calendar row primary key |
| `equipment_id` | `INT UNSIGNED` | NULL, FK -> tbl_equipment | Equipment if capacity is equipment-specific |
| `station_name` | `VARCHAR(100)` | NOT NULL | Factory station name |
| `calendar_date` | `DATE` | NOT NULL | Capacity date |
| `shift_code` | `VARCHAR(30)` | NOT NULL | Shift |
| `available_minutes` | `INT UNSIGNED` | NOT NULL | Net available time |
| `rated_capacity_qty` | `DECIMAL(18,4)` | NULL | Capacity quantity for the shift |
| `capacity_uom` | `VARCHAR(20)` | NULL | Capacity unit |
| `maintenance_block_minutes` | `INT UNSIGNED` | NOT NULL DEFAULT 0 | Planned maintenance block |

**Indexes:** `UNIQUE(station_name, calendar_date, shift_code)`, `INDEX(equipment_id, calendar_date)`

---

### tbl_manufacturing_exceptions

**Purpose:** Planning, material, capacity, and schedule exceptions requiring owner action.

**Key columns:**

| Column | Type | Constraint | Description |
|---|---|---|---|
| `exception_id` | `BIGINT UNSIGNED` | PK, AUTO_INCREMENT | Exception primary key |
| `order_id` | `INT UNSIGNED` | NULL, FK -> tbl_production_orders | Related production order |
| `exception_type` | `ENUM('material_shortage','capacity_overload','late_order','quality_hold','layout_congestion','resource_variance')` | NOT NULL | Exception category |
| `stock_item_id` | `INT UNSIGNED` | NULL, FK -> tbl_stock_items | Material if relevant |
| `required_date` | `DATE` | NULL | Required date |
| `shortage_qty` | `DECIMAL(18,4)` | NULL | Shortage quantity |
| `suggested_action` | `VARCHAR(100)` | NULL | Suggested action |
| `owner_user_id` | `INT UNSIGNED` | NULL, FK -> tbl_users | Current owner |
| `status` | `ENUM('open','in_progress','resolved','cancelled')` | NOT NULL DEFAULT 'open' | Exception status |
| `due_date` | `DATE` | NULL | Action due date |
| `created_at` | `DATETIME` | NOT NULL DEFAULT CURRENT_TIMESTAMP | |

**Indexes:** `INDEX(exception_type, status, due_date)`, `INDEX(order_id)`, `INDEX(owner_user_id, status)`

---

### tbl_production_resource_readings

**Purpose:** Operation-level energy, water, time, scrap, rework, and by-product recovery readings for green production metrics.

**Key columns:**

| Column | Type | Constraint | Description |
|---|---|---|---|
| `reading_id` | `BIGINT UNSIGNED` | PK, AUTO_INCREMENT | Reading primary key |
| `order_id` | `INT UNSIGNED` | NOT NULL, FK -> tbl_production_orders | Production order |
| `job_card_id` | `INT UNSIGNED` | NULL, FK -> tbl_job_cards | Operation or job card |
| `energy_kwh` | `DECIMAL(18,4)` | NULL | Energy used |
| `water_litres` | `DECIMAL(18,4)` | NULL | Water used |
| `machine_minutes` | `INT UNSIGNED` | NULL | Machine time |
| `labour_minutes` | `INT UNSIGNED` | NULL | Labour time |
| `scrap_qty` | `DECIMAL(18,4)` | NULL | Scrap quantity |
| `rework_qty` | `DECIMAL(18,4)` | NULL | Rework quantity |
| `recovered_byproduct_qty` | `DECIMAL(18,4)` | NULL | Recovered by-product quantity |
| `recorded_by` | `INT UNSIGNED` | NOT NULL, FK -> tbl_users | Recorder |
| `recorded_at` | `DATETIME` | NOT NULL DEFAULT CURRENT_TIMESTAMP | |

**Indexes:** `INDEX(order_id)`, `INDEX(job_card_id, recorded_at)`
