...
This section defines the 4-phase build sequence, module delivery per phase, gate criteria, geographic expansion timeline, and standards traceability for the Medic8 Product Requirements Document.
...
| Phase 2 | Growth | Month 7-12 | 50 facilities (clinics + mission hospitals + HC IVs) | UGX 15M | First government facility; first mission hospital; AI Intelligence module launch |
...
| CDS Engine | Clinical safety | 4-tier alert architecture (Info / Warning / Serious / Fatal); override logging with clinician ID, timestamp, reason, patient ID, alert ID; per-facility override rate tracking |

Multi-language delivery: English, French, and Kiswahili string files for all Phase 1 modules shall be 100% complete with zero `[I18N-GAP]` tags before the Phase 1 production deployment. Translation completeness is a Phase 1 gate criterion, not a post-launch task.
...
- i18n architecture in place from day 1: Laravel `lang/en/`, `lang/fr/`, `lang/sw/`; Android `values/strings.xml`, `values-fr/strings.xml`, `values-sw/strings.xml`; iOS `en.lproj/`, `fr.lproj/`, `sw.lproj/`.
...
9. French and Kiswahili string files 100% complete with zero `[I18N-GAP]` tags across all Phase 1 modules before first production deployment.
...
| Insurance Management | Financial | Pre-authorisation workflow; claims submission and tracking; reconciliation with 3+ Uganda insurers (AAR, Jubilee, NHIS when available); AI Claim Scrubbing panel at claim review step |
...
| Full Laboratory | Clinical | HL7 v2 analyser interfaces (ORM/ORU); ASTM E1394 for older analysers; full result management; specimen tracking; CPHL interface; feeds AI Outbreak Early Warning |
...
| Medical Records | Administrative | Record retrieval; filing tracking; record completeness auditing; AI ICD Coding Assist at archival step |
...
| AI Intelligence | AI Add-On | 6 tenant-configurable AI capabilities: AI Clinical Documentation, AI ICD Coding Assist, AI Differential Diagnosis, AI Patient Plain-Language Summary, AI Claim Scrubbing, AI Outbreak Early Warning; provider-agnostic adapter (OpenAI, Anthropic, DeepSeek, Gemini); credit pack and flat fee billing models |

AI Intelligence is available from Phase 2. It is not deferred to Phase 3. Any facility on any clinical tier (Starter, Growth, Pro, Enterprise) may activate the AI Intelligence module independently of their clinical subscription tier.
...
7. AI Intelligence module live with at least 1 pilot facility; AI success metrics baseline established.
...
| Patient Mobile App | Patient engagement | Appointment booking; lab result viewing; medication reminders; Patient Activation Measure (PAM) scoring; prescription refill requests; Android 7.0+ / iOS 15.0+; AI Patient Plain-Language Summary in patient's preferred locale |
...
| Kenya | Phase 3 | Kenya DPA 2019, PPB, Kenya MOH 105 | Kiswahili localisation (already built); Kenya NHIF integration; PPB drug formulary; Huduma Namba patient ID | Private clinics, mission hospitals |
| Tanzania | Phase 3 | EPOCA, TMDA | Kiswahili localisation (already built); NHIF Tanzania; TMDA drug scheduling | Private clinics, mission hospitals |
| Rwanda | Phase 3/4 | Law No. 058/2021, RBC | French localisation (already built); RBC health data standards; community health insurance (Mutuelle de Santé) | Mission hospitals, government facilities |
| DRC | Phase 4 | DRC data protection framework | French localisation (already built); extreme offline-first; CDF currency | NGO/mission hospitals |
...
English, French, and Kiswahili are the 3 launch languages, all required at Phase 1 ship. Expansion to additional languages (Kinyarwanda for Rwanda, Hausa/Igbo for Nigeria, Hindi for India) will be considered post-Phase 4, based on the following criteria:

- Market entry decision confirmed for the target country.
- Sufficient Monthly Recurring Revenue from that market to fund a native-speaker clinical translation review process.
- A formal localisation sign-off process — including clinical terminology review by a native-speaker clinician — completed before any additional language is merged to the `release` branch.

No additional language is added without a formal localisation sign-off. Machine translation is not used at any stage.
...
| 9 Roadmap | 6.4.1.5 (Priorities); 6.4.2.3 (User requirements — operational scenarios) | 4.4 (System modes and states — phased capability delivery) | 4-phase build sequence with gate criteria |
