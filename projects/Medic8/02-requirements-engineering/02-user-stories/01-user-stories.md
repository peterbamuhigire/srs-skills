...
1. The AI draft is generated within 8 s of the clinician clicking the **Draft with AI** button.
2. The draft is displayed in a clearly labelled "AI Draft" panel with **Approve** and **Discard** actions.
3. Clicking **Approve** saves the draft to the patient record; clicking **Discard** removes it without saving.
4. The draft is never saved without the clinician explicitly clicking **Approve**.
5. An audit log entry records the draft generation, approval or discard, clinician ID, and token count.
...
1. Suggestions appear as a selectable list alongside the diagnosis field.
2. The top 3–5 suggestions are shown, each with a confidence score.
3. The clinician can select a suggestion or dismiss all and type a manual search query.
4. Selecting a suggestion adds the ICD code to the diagnosis list.
5. Acceptance and rejection events are logged for acceptance rate reporting.
...
1. The differential list appears as a collapsible panel below the presenting complaint field.
2. Each entry shows: condition name, ICD-11 code, top 3 contributing factors, and rank position.
3. The panel header reads: "AI Differential — for clinician review only. Not a diagnosis."
4. The clinician can dismiss individual suggestions; dismissed suggestions are not written to the record.
5. No suggestion is written to the patient record unless the clinician actively selects it as a diagnosis.
...
1. The summary is displayed in the patient portal app within 10 s of the clinician approving the discharge.
2. The summary is rendered in the patient's preferred locale (set in their profile).
3. The summary contains no ICD codes, drug abbreviations, or clinical acronyms.
4. A "This summary was generated by AI and reviewed by your care team" disclosure appears below the summary.
...
1. Before submission, the system displays a "Claim Risk Analysis" step showing risk indicators per line item.
2. High-risk items are highlighted in red with the specific rejection reason predicted.
3. Medium-risk items are highlighted in amber.
4. The liaison can proceed to submit despite warnings (override is permitted with acknowledgement).
5. The AI scrub completes within 8 s of the liaison clicking **Analyse Claim**.
...
1. An alert is generated when the number of diagnoses for a single disease code exceeds 2 standard deviations above the 90-day rolling baseline for that code at this facility.
2. The alert names the disease code, the current count, the baseline, and the deviation.
3. The alert is delivered to the Medical Officer via in-app notification and SMS within 5 min of the threshold being crossed.
4. The Medical Officer can dismiss the alert with a reason, or escalate to the District Health Officer.
5. False positive rate is tracked and reported in the AI admin panel.
...
1. The AI admin panel shows a provider selection dropdown: OpenAI, Anthropic, DeepSeek, Gemini.
2. The selected provider's API key is entered in an encrypted field and saved without ever being displayed in plain text again.
3. A secondary (failover) provider can be configured independently.
4. The administrator can test the connection by sending a health check request and viewing the response status.
5. Provider changes take effect within 60 s of saving.
...
1. The admin panel shows: current credit balance, estimated days remaining at current consumption rate, and a usage breakdown by capability for the current billing period.
2. A configurable low-balance alert triggers an email to the AI Administrator when balance falls below a user-set threshold.
3. AI features are paused automatically when credits reach zero; clinical features are unaffected.
4. A **Top Up Credits** button links to the billing portal.
...
1. The receptionist can select Kiswahili from a language selector in their profile settings.
2. All UI labels, buttons, form fields, and navigation items in the registration and OPD modules render in Kiswahili after the locale is changed.
3. The locale change takes effect within 500 ms without a full page reload.
4. Clinical alert severity labels (Fatal, Serious, Warning, Info) render in Kiswahili for a clinician whose preferred locale is `sw`.
5. No English fallback strings are visible for any string that has an approved Kiswahili translation.
...
1. The patient portal detects the patient's preferred locale (`fr`) from their profile.
2. Appointment reminder SMS messages are sent in French.
3. Lab result summaries in the patient portal are displayed in French.
4. The AI plain-language summary of discharge notes is generated in French at an appropriate reading level.
