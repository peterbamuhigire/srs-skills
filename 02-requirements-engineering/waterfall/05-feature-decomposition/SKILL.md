...
3. Write exactly one "shall" per clause; pair every stimulus with a deterministic response. Include explicit error handling requirements that are independently verifiable.
4. Confirm Section 3.2 references ISO/IEC 25010 Functional Suitability and that each requirement is traceable back to a feature in `features.md`.
