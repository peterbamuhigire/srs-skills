---
name: mobile-rbac
description: Role-Based Access Control for Android mobile apps integrating with a
  multi-tenant SaaS backend. Covers permission fetching, caching in EncryptedSharedPreferences,
  Jetpack Compose permission gates (PermissionGate, ModuleGate, PermissionButton)...
metadata:
  portable: true
  compatible_with:
  - Codex
...
- The task is unrelated to `mobile-rbac` or would be better handled by a more specific companion skill.
...
|----------|----------|--------|---------|
| Security | Android RBAC permission matrix | Markdown doc mapping roles to feature gates and screen visibility | `docs/android/rbac-matrix-checkout.md` |
| Security | RBAC enforcement test plan | Markdown doc covering positive + negative permission scenarios | `docs/android/rbac-tests.md` |
