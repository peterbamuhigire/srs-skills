...
1. Verify `projects/<ProjectName>/_context/vision.md` exists. Optionally check for `projects/<ProjectName>/<phase>/<document>/PRD.md` and `projects/<ProjectName>/<phase>/<document>/Vision_Statement.md` to enrich the analysis.
2. Invoke `logic.prompt` or trigger the skill. The skill reads context files, generates financial analysis with LaTeX formulas, and writes `projects/<ProjectName>/<phase>/<document>/Business_Case.md`.
