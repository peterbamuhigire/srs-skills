 ...
 *
 * Ported verbatim (logic unchanged) from ECC's
 * `scripts/hooks/plan-canvas-pending.js` (C:\Users\Peter\Downloads\ECC-main)
 * into this engine's `hooks/`, alongside `destructive-bash-gate.js`. Only
 * the CLI entrypoint at the bottom of this file is new — see the comment
 * there for why.
 */
...
// --- CLI entrypoint ------------------------------------------------------
//
// Ported from ECC's `scripts/hooks/plan-canvas-pending.js`
// (C:\Users\Peter\Downloads\ECC-main). ECC invokes this module's `run()`
// through a plugin-root-resolution bootstrap wrapper baked into ECC's own
// hooks.json. This engine has no such bootstrap (see
// `hooks/destructive-bash-gate.js` for the established pattern), so this
// entrypoint reads stdin directly and prints the hook's stdout/exit code
// itself, the same way `destructive-bash-gate.js` does for PreToolUse.
//
// Contract (Claude Code Stop hook): stdin is the JSON payload
// (session_id, transcript_path, cwd, stop_hook_active); stdout, when it
// contains `{"decision":"block","reason":...}`, blocks the stop and hands
// the reason to the agent as its next input; exit code 0 either way (this
// hook never uses the exit-code-2 block form — it blocks via stdout JSON
// per `run()`'s contract, matching ECC's own semantics).
function readStdin() {
  try {
...
if (require.main === module) {
  run(readStdin())
    .then(result => {
      ...
      process.exit((result && result.exitCode) || 0);
    })
    .catch(() => {
