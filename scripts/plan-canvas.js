...
/**
 * Plan Canvas CLI — open governance artefacts (SRS sections, ADRs,
 * acceptance-criteria catalogues) in a browser review canvas and block on
 * human feedback.
 *
 *   node scripts/plan-canvas.js open projects/Foo/09-governance-compliance/05-adr/0007-x.md
 ...
 *
 * Ported from ECC's `scripts/plan-canvas.js` (C:\Users\Peter\Downloads\ECC-main)
 * into this engine's own `scripts/` the same way `install-engine.js` and
 * `generate-plugin-manifest.js` were vendored: no dependency on ECC's
 * package.json, plugin-root resolution, or bootstrap wrappers. The only
 * change from the ECC original is the VERSION constant below (a static
 * string instead of a `require('../package.json').version` read, since this
 * engine has no package.json at its root).
 */
